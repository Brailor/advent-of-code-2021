...
let left_up_corner (x, y) = [|(x + 1, y); (x, y + 1); (x + 1, y + 1)|]
let left_bottom_corner (x, y) = [|(x + 1, y); (x, y - 1); (x + 1, y - 1)|]
let right_up_corner (x, y) = [|(x - 1, y); (x, y + 1); (x - 1, y + 1)|]
let right_bottom_corner (x, y) = [|(x - 1, y); (x, y - 1); (x - 1, y - 1)|]
let first_row_neighbors (x, y) = [|(x - 1, y); (x + 1, y); (x, y + 1); (x - 1, y + 1); (x + 1, y + 1)|]
let first_col_neiggbors (x, y) = [|(x, y + 1); (x, y - 1); (x + 1, y); (x + 1, y - 1); (x + 1, y + 1)|]
let last_row_neighbors (x, y) = [|(x, y - 1); (x - 1, y); (x + 1, y); (x - 1, y - 1); (x + 1, y - 1)|]
let last_col_neighbots (x, y) = [|(x, y + 1); (x, y - 1); (x - 1, y); (x - 1, y - 1); (x - 1, y + 1)|]
let neighbors (x, y) = 
...
let get_element (x:int, y:int) (matrix: int[,]) =
    (y, x) ||> Array2D.get matrix

let problem1 =
    let rec flash_neighbor (matrix: int[,]) (nx: int, ny :int) =
        let neighbors = get_neighbors (nx, ny)
        neighbors |> Array.iter (fun (nx, ny) ->
        ...
        let rec flash_neighbor cloned= 
            cloned 
                |> Array2D.iteri (fun y x v ->
                    if v > 9 && not (already_flashed |> Set.contains (x, y)) then
                        ...
                        neighbors |> Array.iter (fun (nx, ny) ->
                            ...
                            if (neighbor_val > 9) && not (already_flashed |> Set.contains (nx, ny)) then
    ...
    let rec stepper (matrix: int[,]) (step: int) (flash_count: int) =
        ...
        | _ ->
            ...
            let flattend = flat2Darray m |> Seq.forall (fun elem -> elem = 0)
